...
class Arty200THarness(override implicit val p: Parameters) extends Arty200TShell {
  ...
  println(s"Arty200T FPGA Base Clock Freq: ${dutFreqMHz} MHz")
  ...
  val ddrOverlay = dp(ExtTLMem).map { extMem =>
    dp(DDROverlayKey).head
      ...
      .asInstanceOf[DDRArty200TPlacedOverlay]
  }
