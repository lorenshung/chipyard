...
# --                |         |         |
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2025/05/28     |JH       |         |- setup_extended_config search with ID instead of ProdID
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2025/01/27     |JH       |         |- update setup_extended_config: allow multiple files now, separator |
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2023/08/10     |JH       |         |- add new environment variable update TE_GUI_DISABLED
# --                |         |         |- new change history style
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2022/09/22     |JH       |         |- typo
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2022/09/13     |JH       |         |- simple bugfix printf for source_file
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2021/12/16     |JH       |         |- add scan EDITOR and WSL variable
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2021/03/09     |JH       |         |- prevent board store to install all files and select only give one to install
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2020/08/12     |JH       |         |- add notes for board store
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2020/05/05     |JH       |         |- add putty path and ignore list
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2020/04/07     |JH       |         |- add serial path
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2020/03/30     |JH       |         |- optional xilinx boardstore
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2018/09/17     |JH       |         |- add internal betaversion of xilinx boardstore
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2017/09/04     |JH       |         |- add new document history style
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2017/02/02     |JH       |         |- miscellaneous
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2016/02/03     |JH       |         |- initial release
# ------------------|---------|---------|----------------------------------------------------------------------
# --------------------------------------------------------------------
namespace eval ::TE {
  namespace eval ENV {
    ...
    proc set_add_env {} {
      #general vivado which use temporary envirments
      ...
      #todo remove sodsoc in future releases
      ...
      # serial
      if {[catch {set TE::TE_GUI_DISABLED  ${::env(TE_GUI_DISABLED)}}]} {
        set TE::TE_GUI_DISABLED 0
      }
      set tmpdir [pwd]
      cd $TE::LOG_PATH 
    ...
    #-- - replace variant depended PLL settings, see /board_files/*_board_files.csv
    proc setup_extended_config {} {
      set config_sw_extpll [TE::BDEF::get_config_sw_extpll $TE::ID  ${TE::BDEF::ID}]
      # puts "Test---------------------------------" 
      # puts "Test $config_sw_extpll" 

      if {![string match -nocase "NA" $config_sw_extpll ] } {
        set source_files [split $config_sw_extpll "|"]
        foreach file $source_files {
          set source_file "$TE::BASEFOLDER/$file"
          if { [file exists $source_file] } { 
            set filename [file tail $source_file]
            #replace file in SW Lib folder
            set destination_files []
            if { [catch {set destination_files [TE::UTILS::findFiles $TE::LIB_PATH $filename]}] } {
              set destination_files []
              TE::UTILS::te_msg TE_INIT-233 {CRITICAL WARNING} "Extension destination not available, please check : $filename in $TE::LIB_PATH"
            }
            ...
            #replace file in petalinux Lib folder
            set destination_files []
            if { [catch {set destination_files [TE::UTILS::findFiles $TE::PETALINUX_PATH $filename]}] } {
              set destination_files []
              TE::UTILS::te_msg TE_INIT-246 {CRITICAL WARNING} "Extension destination not available, please check : $filename in $TE::PETALINUX_PATH"
            }
            foreach des_file $destination_files { 
              file copy -force ${source_file} ${des_file}
              TE::UTILS::te_msg TE_INIT-247 INFO "Replace  $des_file with $source_file"
            }
          } else {
            TE::UTILS::te_msg TE_INIT-235 ERROR "Extension source not available, please check : $source_file"
          }
        }
      }
