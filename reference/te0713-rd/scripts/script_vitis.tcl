...
# --                |         |         |
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2025/05/28     |JH       |         |- vitis_main, run_all, load environment.tcl and start PLL header copy
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2025/03/28     |MK       |         |- treat warning as errors by default
# ------------------|---------|---------|----------------------------------------------------------------------                                                                                                           
  ...
  namespace eval VITIS {
    ...
    proc create_linux_source { domainname } { 
      if {$TE::VITIS::SYSTEM eq "zynq"} {
        puts "Warning: (TE) Create Linux sources currently not implemented for $TE::VITIS::SYSTEM"
      } elseif {$TE::VITIS::SYSTEM eq "zynqMP"} {
        puts "Warning: (TE) Create Linux sources currently not implemented for $TE::VITIS::SYSTEM"
      } elseif {$TE::VITIS::SYSTEM eq "versal"} {
        puts "Warning: (TE) Create Linux sources currently not implemented for $TE::VITIS::SYSTEM"
      ...
      # if {![string match "NA" $linux_path]} {
        ...
        # set tmp_bitstream [TE::UTILS::prebuilt_file_location true * $ext NA NA NA false]
        ...
        # } elseif {$TE::VITIS::SYSTEM eq "versal"} {
          # set tmp_plm [TE::UTILS::prebuilt_file_location true  plm .elf NA NA $TE::WORKSPACE_SDK_PATH false]
          # set tmp_psm [TE::UTILS::prebuilt_file_location true  psmfw .elf NA NA $TE::WORKSPACE_SDK_PATH false]
          
          # if {[file exists $tmp_plm ]} {
            # file copy -force ${tmp_plm} ${boot_loc} 
            # set bif_plm "<plm.elf>"
          # } else {
            # set bif_plm "${TE::WORKSPACE_SDK_PATH}/${TE::PRODID}/export/${TE::PRODID}/sw/${TE::PRODID}/boot/plm.elf"
          # }
          # if {[file exists $tmp_psm ]} {
            # file copy -force ${tmp_psm} ${boot_loc} 
            # set bif_psm "<pmufw.elf>"
          # } else {
            # set bif_psm "${TE::WORKSPACE_SDK_PATH}/${TE::PRODID}/export/${TE::PRODID}/sw/${TE::PRODID}/boot/pmufw.elf"
          # }
          # TE::UTILS::vitis_zmp_bif -biffile ${bif_loc}/boot.bif -linux true -bootloader $bif_fsbl -pmu $bif_pmu
          
          # TE::UTILS::vitis_versal_bif -biffile ${bif_loc}/boot.bif -linux true \
           # -plm_use true -plm $bif_plm \
           # -psmfw_use true -psmfw $bif_psm \
        ...
        # } elseif {$TE::VITIS::SYSTEM eq "microblaze"} {
          # #todo
          # puts "Warning:(TE) linux file ${TE::PRODID} failed: Create Microblaze linux domain not implemented $TE::VITIS::SYSTEM"
        # } else {
          # puts "Error:(TE) linux file ${TE::PRODID} failed: unknown system type $TE::VITIS::SYSTEM"
        # }
        
        # if {$TE::VITIS::SYSTEM eq "zynq" || $TE::VITIS::SYSTEM eq "zynqMP" || $TE::VITIS::SYSTEM eq "versal"} {
      ...
      foreach sw_platlist_line $TE::SDEF::SW_DOMLIST {
        if {[catch {
          ...
          if { [lindex $sw_platlist_line ${TE::SDEF::D_ID}] eq "0" } {
            ...
            set d_proc [TE::VITIS::translate_processorname [lindex $sw_platlist_line ${TE::SDEF::D_PROC}]]
    ...
    proc get_domain_name {proc_name os_name} {
      ...
      if { $os_name eq "zynq_fsbl" || $os_name eq "zynqmp_fsbl" || $os_name eq "zynqmp_pmufw" || $os_name eq "flash_fsbl"} { 
        ...
        foreach dom $dlist {
         ...
         set tmp_pr [TE::VITIS::translate_processorname $proc_name]
        ...
        if {[catch {
          if { [lindex $sw_applist_line ${TE::SDEF::ID}] ne "id" } {
            ...
            if { [lindex $sw_applist_line ${TE::SDEF::STEPS}] eq "0" || [lindex $sw_applist_line ${TE::SDEF::STEPS}] eq "3" || [lindex $sw_applist_line ${TE::SDEF::STEPS}] eq "FSBL_APP"} {
              ...
              domain active $domainname
              set app_proc [TE::VITIS::translate_processorname $app_proc]
              ...
              if {[string match $name $app_name]} {
                ...
                configapp -app ${app_name} compiler-misc {-Werror -Wall -Wextra}
    ...
    proc app_delete {{name *}} {
      ...
      foreach element $tmplist {
        if {[catch {
          ...
          puts "Error:(TE) Script (TE::VITIS::app_delete) failed  at $element with: $result."
   ...
   #--------------------------------
    #--run_all:
    proc run_all {{deep 3}} {
      puts "Info:(TE) Set Update PLL Config Header..."
    ...
    proc vitis_main {} {
      ...
      for {set option 0} {$option < $tmp_argc} {incr option} {
       ...
       source ${TE::VITIS::SCRIPT_PATH}/script_environment.tcl
