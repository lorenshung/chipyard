...
@if %VIVADO_AVAILABLE%==1 (
  call vivado -source ../scripts/script_main.tcl  -mode batch -notrace -tclargs --program_bit --boardpart %PARTNUMBER% --program_swapp %SWAPP% --use_basefolder %PROGRAM_ROOT_FOLDER_FILE%
) else (
  call vivado_lab -source ../scripts/script_main.tcl  -mode batch -notrace -tclargs --program_bit --boardpart %PARTNUMBER% --program_swapp %SWAPP% --use_basefolder %PROGRAM_ROOT_FOLDER_FILE% --labtools
)
