...
# --                |         |         |
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2025-01-27     |JH       |         |- bugfix te_msg: error msg returns error code now 
# --                |         |         |- update te_msg cnt   
# --                |         |         |- remove old te_msg code
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2023-12-08     |JH       |x        |- change path normalisation in prebuilt_file_location (fixed bug for paths with one character)
# --                |         |         |.changes history tyle to new format
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2023-06-06     |JH       |         |- bugfix  vitis_versal_bif for linux
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2023-05-31     |JH       |         |- add vitis_versal_bif, change copy_hw_files
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2023-05-25     |JH       |         |- start versal support, prebuilt_file_location,  copy_hw_files
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2023-08-14     |JH       |         |- increase msg cnt
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2023-06-16     |JH       |         |- unified message with script intel version...backward compatible to older one
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2022-xx-xx     |JH       |         |- update te_msg cnt
# --                |         |         |- update te_msg cnt 
# --                |         |         |- update te_msg cnt 
# --                |         |         |- update te_msg cnt 
# --                |         |         |- notes prod_config
...
# --                |         |         |- update te_msg cnt 
# --                |         |         |- update te_msg cnt 
# --                |         |         |- update vitis_zmp_bif vitis_z_bif with new features and bugfixes
# --                |         |         |- update te_msg cnt 
# --                |         |         |- update te_msg cnt 
# --                |         |         |- copy_sw_files --> new feature for microblaze, replace directly apps in firmware folder,  regenerate .bit and .mcs on hw folder and update vitis bit
# --                |         |         |- update te_msg cnt 
# --                |         |         |- update te_msg cnt 
# --                |         |         |- update te_msg cnt 
# --                |         |         |- vitis_zmp_bif, vitis_z_bif remove special boot.scr and device tree entry
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2021-xx-xx     |JH       |         |- add generate_workspace_petalinux
...
# --                |         |         |- update te_msg cnt 
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2020-xx-xx     |JH       |         |-  add get_bd_vivado_version
...
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2019-xx-xx     |JH       |         |- rise te_msg cnt, add prebuilt_file_location
...
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2018-xx-xx     |JH,LK    |         |- rise te_msg cnt
# --                |         |         |- rise te_msg cnt
# --                |         |         |- rise te_msg cnt
# --                |         |         |- add Dos2Unix
# --                |         |         |- rise te_msg cnt
...
# --                |         |         |- rise te_msg cnt
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2017-xx-xx     |JH,ZP    |         |- miscellaneous
...
namespace eval ::TE {
  namespace eval UTILS {
      ...
      if {$num == 3} {
        ...
        if {$TYPE ne "NA"} { # for better overview in log file
          switch -nocase $TYPE { 
            ...
            "info"				{set TYPE "INFO";}
            "warning"			{set TYPE "WARNING";}
            "critical_warning"	{set TYPE {CRITICAL WARNING};}
            "error"				{set TYPE "ERROR";}
            default 			{set TYPE "ERROR";}
          }
        }
      }
      if {[catch {common::send_msg_id "$ID" $TYPE $MESSAGE}] } {
        puts "${TYPE}: ($ID) $MESSAGE"
        if {[string match -nocase "ERROR" $TYPE] } {
          return -code error
        }
      }
    ...
    if {[string match $typ ".pdi"]  ||[string match $typ ".bit"]  || [string match $typ ".mmi"] || [string match $typ ".ltx"]} {
   ...
   # puts "----------Test:$typ | [file exists ${baseFolder}/${osPath}/${shortDir}/${appPath}]|${baseFolder}/${osPath}/${shortDir}/${appPath} "
    ...
    if {[llength $absPath] > 1} {
     ...
     if {$path_only} {
      ...
      if {$rel} {
        ...
        return [file normalize $absPathOnly]
       }
     } else {
    ...
    proc copy_hw_files { {deleteOldFile  true}} {
      ...
      if {[file exists ${TE::VPROJ_PATH}/${TE::VPROJ_NAME}.runs/${TE::IMPL_NAME}/${TE::PR_TOPLEVELNAME}.bit]} {
        ...
        if {!$TE::IS_ZSYS && ! $TE::IS_ZUSYS && ! $TE::IS_VSYS} {
        ...
        TE::UTILS::create_prebuilt_report HW
      } elseif {[file exists ${TE::VPROJ_PATH}/${TE::VPROJ_NAME}.runs/${TE::IMPL_NAME}/${TE::PR_TOPLEVELNAME}.pdi]} {
          set hw_path [TE::UTILS::prebuilt_file_location false * .pdi NA NA NA true]
          if {[string match "NA" $hw_path] } { 
            set hw_path ${TE::PREBUILT_HW_PATH}/${TE::SHORTDIR}
            file mkdir $hw_path
          }
          ...
          #delete old prebuilt bitfile
          if {[file exists ${hw_path}/${TE::PB_FILENAME}.pdi] && $deleteOldFile } {
            if {[catch {file delete -force ${hw_path}/${TE::PB_FILENAME}.pdi} result]} {TE::UTILS::te_msg TE_UTIL-?? {CRITICAL WARNING} " $result"}
          }
          
          file copy -force ${TE::VPROJ_PATH}/${TE::VPROJ_NAME}.runs/${TE::IMPL_NAME}/${TE::PR_TOPLEVELNAME}.pdi ${hw_path}/${TE::PB_FILENAME}.pdi
          TE::UTILS::te_msg TE_UTIL-?? INFO "${hw_path}/${TE::PR_TOPLEVELNAME}.pdi was replaced with  ${TE::VPROJ_PATH}/${TE::PB_FILENAME}.pdi"
            
          #--------------------------------
          #delete old prebuilt lpr
          if {[file exists ${hw_path}/${TE::PB_FILENAME}.lpr] && $deleteOldFile } {
            if {[catch {file delete -force ${hw_path}/${TE::PB_FILENAME}.lpr} result]} {TE::UTILS::te_msg TE_UTIL-?? {CRITICAL WARNING} " $result"}
          }
          #copy and rename lpr
          file copy -force ${TE::VPROJ_PATH}/${TE::VPROJ_NAME}.hw/${TE::VPROJ_NAME}.lpr ${hw_path}/${TE::PB_FILENAME}.lpr
          TE::UTILS::te_msg TE_UTIL-31 INFO "${hw_path}/${TE::PB_FILENAME}.lpr was replaced with ${TE::VPROJ_PATH}/${TE::VPROJ_NAME}.hw/${TE::VPROJ_NAME}.lpr"
          ...
          #delete old prebuilt ltx_file
          if {[file exists ${hw_path}/${TE::PB_FILENAME}.ltx] && $deleteOldFile } {
            if {[catch {file delete -force ${hw_path}/${TE::PB_FILENAME}.ltx} result]} {TE::UTILS::te_msg TE_UTIL-?? {CRITICAL WARNING} " $result"}
          }
          if {[file exists ${TE::VPROJ_PATH}/${TE::VPROJ_NAME}.runs/${TE::IMPL_NAME}/debug_nets.ltx]} {
            file copy -force ${TE::VPROJ_PATH}/${TE::VPROJ_NAME}.runs/${TE::IMPL_NAME}/debug_nets.ltx ${hw_path}/${TE::PB_FILENAME}.ltx
            TE::UTILS::te_msg TE_UTIL-?? INFO "${hw_path}/${TE::PB_FILENAME}.ltx was replaced with ${TE::VPROJ_PATH}/${TE::PB_FILENAME}.runs/${TE::IMPL_NAME}/debug_nets.ltx"
          } else {TE::UTILS::te_msg TE_UTIL-?? INFO "${TE::VPROJ_PATH}/${TE::VPROJ_NAME}.runs/${TE::IMPL_NAME}/debug_nets.ltx does not exist."}
          #--------------------------------
          #delete old prebuilt xsa
          if {[file exists ${hw_path}/${TE::PB_FILENAME}.xsa]  && $deleteOldFile } {
            if {[catch {file delete -force ${hw_path}/${TE::PB_FILENAME}.xsa} result]} {TE::UTILS::te_msg TE_UTIL-?? {CRITICAL WARNING} " $result"}
          }
          #copy and rename xsa
          if {[file exists ${TE::VPROJ_PATH}/${TE::PB_FILENAME}.xsa]} {
            #optional only on processor system: check bd file name --> for fsys no *hwdef and *sydef files needed
            file copy -force  ${TE::VPROJ_PATH}/${TE::PB_FILENAME}.xsa ${hw_path}/${TE::PB_FILENAME}.xsa
            TE::UTILS::te_msg TE_UTIL-?? INFO "${hw_path}/${TE::PB_FILENAME}.xsa was replaced with ${TE::VPROJ_PATH}/${TE::PB_FILENAME}.xsa"
          } else {TE::UTILS::te_msg TE_UTIL-?? {CRITICAL WARNING} "${TE::VPROJ_PATH}/${TE::PB_FILENAME}.xsa does not exist."}
          
          TE::UTILS::create_prebuilt_report PDI
          
      } else {TE::UTILS::te_msg TE_UTIL-48 {CRITICAL WARNING} "${TE::VPROJ_PATH}/${TE::VPROJ_NAME}.runs/${TE::IMPL_NAME}/${TE::PR_TOPLEVELNAME}.bit or *.pdi does not exist. Nothing was copied to prebuilt folder."}     
    }  
    #--------------------------------
    #--copy_sw_files:   
    proc copy_sw_files {} {
      ...
      foreach app $applications {
        ...
        #not all temp elf reused deleted
        if {![string match  "executable.elf" ${filenameonly}] && ![string match */resources/* ${app}] && ![string match */zynqmp_pmufw/* ${app}] && ![string match */zynqmp_fsbl/* ${app}] && ![string match */qemu/* ${app}] && ![string match "u-boot.elf" ${filenameonly}]  && ![string match "bl31.elf" ${filenameonly}]} {
          ...
          #for microblaze elfs only..copy directly to the firmware folder and regenerate mcs
          if {[file exists ${TE::FIRMWARE_PATH}]} {
            set test [TE::UTILS::findFiles ${TE::FIRMWARE_PATH} $filenameonly]
            foreach firmare_app $test {
              #replace firmware folder
              set firmfolder [file dirname $firmare_app]
              set firmfilenameonly [file tail $firmare_app]
              TE::UTILS::te_msg TE_UTIL-174 STATUS "Update $firmare_app"
              if {[catch {file copy  -force $app ${firmfolder}} result]} {TE::UTILS::te_msg TE_UTIL-173 {CRITICAL WARNING} " $result"}
              #update bitstream on the prebuilt folder
              #todo den Teil etwas generischer und einzelfunktion oder mit TEX::EXT::generate_app_bit_mcs vereinen
              #1 processor name from firmware path
              set procname [TE::UTILS::relTo $firmfolder ${TE::FIRMWARE_PATH}]
              set procname [string map {./ ""} $procname]
              set procname "msys_i/${procname}"
              if { [string match -nocase *MCS* $procname]} {
                set procname "${procname}/U0/_microblaze_I"
              }
              # puts "TestJH4--TODO $procname"
              set mmi_abs [TE::UTILS::prebuilt_file_location false ${TE::PB_FILENAME} .mmi NA NA NA false]
              # puts "TestJH5--TODO $mmi_abs"
              set elf_abs [TE::UTILS::prebuilt_file_location false ${filenameonly} .elf NA NA NA false]
              # puts "TestJH6--TODO $elf_abs"
              set bit_abs [TE::UTILS::prebuilt_file_location false ${TE::PB_FILENAME} .bit NA NA NA false]
              # puts "TestJH7--TODO $bit_abs"
              #2 update bitstream
              TE::UTILS::te_msg TE_UTIL-176 STATUS "Update $bit_abs with $firmfilenameonly"
              set command exec
              ...
              lappend command -proc $procname
              lappend command -out ${bit_abs}

              set result [eval $command]

              #3 create mcs again
              set mcsname [string map {.bit ".mcs"} $bit_abs]
              TE::UTILS::te_msg TE_UTIL-177 STATUS "Update $mcsname with $firmfilenameonly"
              TE::VIV::write_viv_cfgmem ${bit_abs}  ${mcsname}
              #4 replace .bit on vitis folder
               set test2 [TE::UTILS::findFiles ${TE::WORKSPACE_SDK_PATH} *.bit] 
               foreach f_test2 $test2 {
                TE::UTILS::te_msg TE_UTIL-178 STATUS "Update $f_test2 with $firmfilenameonly"
                if {[catch {file copy  -force ${bit_abs} ${f_test2}} result]} {TE::UTILS::te_msg TE_UTIL-179 {CRITICAL WARNING} " $result"}
              }
            }
          }
        } else {
    ...
    proc create_prebuilt_report { type {app "unknown"}} {
      ...
      } elseif {[string match "$type" PDI]} {
        #*.pid
        set file_check [TE::UTILS::prebuilt_file_location false ${TE::PB_FILENAME} .pdi NA NA $TE::PREBUILT_PATH false]
        if {![string match "NA" $file_check]} {
          set xtime [file mtime $file_check ]
          set date "[ clock format $xtime -format "%Y_%m_%d:%H_%M_%S"]"
          set data_path "[TE::UTILS::prebuilt_file_location true ${TE::PB_FILENAME} .pdi NA NA $TE::PREBUILT_PATH true]"
        }   
    ...
    proc vitis_z_bif {{args ""}} {
      ...
      for {set i 0} {$i < [llength $data_file]} {incr i} {
            ...
            if {[string match "" "[lindex $data_load $i]"] || [string match "NA" "[lindex $data_load $i]"]} {
              puts -nonewline $bif_fp "  \[offset =  [lindex $data_offset $i]\]"
            } else {
              puts -nonewline $bif_fp "  \[offset =  [lindex $data_offset $i],load =  [lindex $data_load $i]\]"
            }   
					}    
    ...
    proc vitis_zmp_bif {{args ""}} {
      ...
      for {set i 0} {$i < [llength $data_file]} {incr i} {
				if {![string match "" "[lindex $data_file $i]"] && ![string match "NA" "[lindex $data_file $i]"]} {
					if {[string match "" "[lindex $data_offset $i]"] || [string match "NA" "[lindex $data_offset $i]"]} {
            puts -nonewline $bif_fp "  \[destination_cpu = $app_cpu,load =  [lindex $data_load $i]\]"
					} else {
            if {[string match "" "[lindex $data_load $i]"] || [string match "NA" "[lindex $data_load $i]"]} {
              puts -nonewline $bif_fp "  \[destination_cpu = $app_cpu,offset =  [lindex $data_offset $i]\]"
            } else {
              puts -nonewline $bif_fp "  \[destination_cpu = $app_cpu,offset =  [lindex $data_offset $i],load =  [lindex $data_load $i]\]"
            }   
					}    
					puts $bif_fp  [lindex $data_file $i]
				}
    ...
    #--------------------------------
    #--vitis_versal_linux_bif:  
    #-- generate generic zynqMP Vitis bif
    #-- returns string:
    proc vitis_versal_bif {{args ""}} {
      set biffile [pwd]/boot.bif
      set linux_bif true
    
      set bootimage_use false
      set bootimage "<image.pdi>"
      ...
      set app_cpu "a53-0"
      set app_exception_level "el-3"
      set app "<elf,versal_cips_0_pspmc_0_psv_cortexa72_0>"
      ...
      for {set option 0} {$option < $args_cnt} {incr option} {
        switch [lindex $args $option] { 
          "-biffile"                { incr option; set biffile [lindex $args $option]}
          "-linux"                  { incr option; set linux_bif [lindex $args $option]}
          "-bootimage_use"          { incr option; set bootimage_use [lindex $args $option]}
          "-bootimage"              { incr option; set bootimage [lindex $args $option]}
          "-plm_use"                { incr option; set plm_use [lindex $args $option]}
          "-plm"                    { incr option; set plm [lindex $args $option]}
          "-psmfw_use"              { incr option; set psmfw_use [lindex $args $option]}
          "-psmfw"                  { incr option; set psmfw [lindex $args $option]}
          "-atf_use"                { incr option; set atf_use [lindex $args $option]}
          "-atf_cpu"                { incr option; set atf_cpu [lindex $args $option]}
          "-atf_exception_level"    { incr option; set atf_exception_level [lindex $args $option]}
          "-atf"                    { incr option; set atf [lindex $args $option]}
      ...
      if { $linux_bif==true && [string match $app "<elf,versal_cips_0_pspmc_0_psv_cortexa72_0>"]} {
        set app_exception_level "el-2"
        set app "<u-boot.elf>"
      }
      set bif_fp [open "$biffile" w]
      ...
      #boot partition
      puts $bif_fp "  image\n  \u007B"
      if {$bootimage_use == true  } {
        puts -nonewline $bif_fp "    \u007B type=bootimage, file="
        puts -nonewline $bif_fp $bootimage
        puts $bif_fp " \u007D"
      }
      if {$plm_use == true  } {
        puts -nonewline $bif_fp "    \u007B type=bootloader, file="
        puts -nonewline $bif_fp $plm
        puts $bif_fp " \u007D"
      }
      if {$psmfw_use == true  } {
        puts -nonewline $bif_fp "    \u007B core=psm, file="
        puts -nonewline $bif_fp $psmfw
        puts $bif_fp " \u007D"
      }
      puts $bif_fp "  \u007D"
      #subsystem partition
      puts $bif_fp "  image\n  \u007B"
      #todo maybe parameter for id and subsystem?
      puts $bif_fp "    id = 0x1c000000, name=apu_subsystem"
      
      if {$linux_bif == true ||$atf_use == true } {
        puts -nonewline $bif_fp "    \u007B core = $atf_cpu, exception_level = $atf_exception_level, file="
        puts -nonewline  $bif_fp $atf
        puts $bif_fp " \u007D"
      }
      ...
      if {$linux_bif == true ||$app_use == true } {
				if {$app_exception_level == ""  ||$app_exception_level == "NA" } {
					puts -nonewline $bif_fp "    \u007B core = $app_cpu, file="
				} else {
					puts -nonewline $bif_fp "    \u007B core = $app_cpu, exception_level = $app_exception_level, file="
				}
        puts  -nonewline $bif_fp $app
        puts $bif_fp " \u007D"
      }
      ...
      for {set i 0} {$i < [llength $data_file]} {incr i} {
				if {![string match "" "[lindex $data_file $i]"] && ![string match "NA" "[lindex $data_file $i]"]} {
					if {[string match "" "[lindex $data_offset $i]"] || [string match "NA" "[lindex $data_offset $i]"]} {
            puts -nonewline $bif_fp "    \u007B type=raw, load =  [lindex $data_load $i], file="
					} else {
            if {[string match "" "[lindex $data_load $i]"] || [string match "NA" "[lindex $data_load $i]"]} {
              puts -nonewline $bif_fp "    \u007B type=raw, offset =  [lindex $data_offset $i], file="
            } else {
              puts -nonewline $bif_fp "    \u007B  type=raw, offset =  [lindex $data_offset $i],load =  [lindex $data_load $i], file="
            }   
					}    
					puts -nonewline $bif_fp [lindex $data_file $i]
          puts $bif_fp " \u007D"
				}
      }
      puts $bif_fp "  \u007D"
