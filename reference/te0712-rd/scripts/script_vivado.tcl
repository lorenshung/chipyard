...
# --                |         |         |
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2024-05-06     |JH       |         |- import_hdl replace - with _ on toplevel name
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2024-01-12     |MK       |         |- add check_bd_typ to hw_program for support of lab edition
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2023-12-22     |MK       |         |- fix hw_get_fpga for multiple devices
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2023-08-15     |JH       |         |- bugfix hw_program_flash versal variable
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2023-08-14     |MKir,JH  |         |- Versal support
# --                |         |         |- sync with 23.1, hw_set_bit_and_ltx_files, hw_fpga_boot_from_memory,hw_program_fpga_device
# --                |         |         |- new change history table
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2023/05/25     |JH       |         |- start add versal support, mod write_viv_cfgmem
# ------------------|---------|---------|----------------------------------------------------------------------
# -- 2022/09/24     |JH       |         |- write_viv_cfgmem reusable for other paths
# ------------------|---------|---------|----------------------------------------------------------------------
# --todo see below  |         |         |
# ------------------|---------|---------|----------------------------------------------------------------------
# -- Change History old todo move to new one:
# ------------------------------------------
      ...
      if {[file exists $TE::HDL_PATH]} {
        ...
        if { [llength $hdl_names] > 0 } {
          ...
          if {[lsearch $hdl_names *${TE::VPROJ_NAME}_top.vhd*]==0} {
            ...
            #Verilog
            # - is not allowed in top level syntax
            set new_name [string map {- _} ${TE::VPROJ_NAME}]
            set_property top ${new_name}_top [current_fileset]
            #overwrite bd_import toplevel_settings
            set TE::PR_TOPLEVELNAME ${TE::VPROJ_NAME}_top
  ...
  namespace eval VLAB {
   # -----------------------------------------------------------------------------------------------------------------------------------------
    ...
    proc hw_get_fpga {{partname ""}} {
      ...
      set part_tmp [split $partname "-"]
      
      set hw_fpga_found false
      
      foreach hwd $tmp {
        set hwd_tmp [split $hwd "_"]
        if {[string match *arm_dap* $hwd ] || [string match $hwd ""] } {
          #ignore arm dap
        } elseif {[string match [lindex $hwd_tmp 0]* [lindex $part_tmp 0]]} {
          #look for direct match
          #tested with versal and artix, others not not tested
          set hw_fpga_found true
          set hw_fpga_name $hwd
          break
        } else {
          #backup case, if no direct match possible, take the one we find
          set hw_fpga_found true
          set hw_fpga_name $hwd
        }
      }
    ...
    proc hw_set_bit_and_ltx_files {use_basefolder app_name {fname ""}} {
      ...
      if {$hw_fpga_name ne "NA"} {
        if {$use_basefolder} {
          ...
          set bitfilename ""
          if { ![catch {set bitfilename [glob -join -dir ${TE::BASEFOLDER}/ ${ext_glob}]}] } {
          ...
          if {$app_name eq "" || $app_name eq "NA"} {
            if {[file exists ${TE::PREBUILT_HW_PATH}/${shortdir_int}/${TE::PB_FILENAME}${ext}]} {
              #use bitfile from hardware folder (with bootloop for microblaze systems)
              set_property PROGRAM.FILE ${TE::PREBUILT_HW_PATH}/${shortdir_int}/${TE::PB_FILENAME}${ext} [lindex [get_hw_devices $hw_fpga_name] 0]
              ...
              TE::UTILS::te_msg TE_PR-7 INFO "Used file:${TE::PREBUILT_HW_PATH}/${shortdir_int}/${TE::PB_FILENAME}${ext}"
              set return_filename  ${TE::PREBUILT_HW_PATH}/${shortdir_int}/${TE::PB_FILENAME}${ext}
            } else {
              #use default bit from hardware folder (with bootloop for microblaze systems)
              if {[file exists ${TE::PREBUILT_HW_PATH}/default/${TE::PB_FILENAME}${ext}]} {
                set_property PROGRAM.FILE ${TE::PREBUILT_HW_PATH}/default/${TE::PB_FILENAME}${ext} [lindex [get_hw_devices $hw_fpga_name] 0]
                ...
                TE::UTILS::te_msg TE_PR-9 INFO "Used file:${TE::PREBUILT_HW_PATH}/default/${TE::PB_FILENAME}${ext}"
                set return_filename  ${TE::PREBUILT_HW_PATH}/default/${TE::PB_FILENAME}${ext}
              } else {
                TE::UTILS::te_msg TE_PR-10 ERROR "${TE::PREBUILT_HW_PATH}/${shortdir_int}/${TE::PB_FILENAME}${ext} doesn't exist. Nothing is done."
                return -code error
              }
            }
          } else {
            ...
            set bitfilename ""
            if { ![catch {set bitfilename [glob -join -dir ${TE::PREBUILT_BI_PATH}/${shortdir_int}/${app_name}/ ${ext_glob}]}] } {
              set_property PROGRAM.FILE ${bitfilename}  [lindex [get_hw_devices $hw_fpga_name] 0]
              ...
              TE::UTILS::te_msg TE_PR-13 ERROR "${TE::PREBUILT_BI_PATH}/${shortdir_int}/${app_name}/*${ext} doesn't exist. Nothing is done."
              return -code error
            }
          }
        }
    ...
    proc hw_program_fpga_device { use_basefolder appname {fname ""}} {
      ...
      if {![catch {set return_filename [hw_set_bit_and_ltx_files $use_basefolder $appname $fname]}] } { 
        ...
        TE::UTILS::te_msg TE_PR-32 INFO "Programming BIT-File or PDI File finished."
        if {[catch {after 500; refresh_hw_device $hw_fpga_name}] } {}
      } else {
    ...
    proc hw_program_flash {{appname ""} {fname ""} {setup 011001} {erase false} {def_fsbl false}} {
      ...
      if {[string match -nocase *xc*7z* $hw_fpga_name] } {
      ...
      } elseif {[string match -nocase *xcv* $hw_fpga_name] }  {
        #Versal
        set binfilename [TE::UTILS::prebuilt_file_location false * .bin NA $appname NA false $shortdir_int]
        set pdi  [TE::UTILS::prebuilt_file_location false * .pdi NA NA NA false $shortdir_int]
        ...
        if {$erase} {
          ...
          if {[catch {set binfilename [glob -join -dir ${TE::BASEFOLDER}/ *.bin]}] } {
             TE::UTILS::te_msg TE_PR-117 {CRITICAL WARNING} "*.bin is not available on ${TE::BASEFOLDER}/"
              set binfilename ""
          }
        } elseif {$binfilename eq "NA"} {
          TE::UTILS::te_msg TE_PR-118 {CRITICAL WARNING} "*.bin is not available in the prebuilt folder."
        }
        set return_filename ${binfilename}
        set_property PROGRAM.FILES [list "${binfilename}" ] [ get_property PROGRAM.HW_CFGMEM [lindex [get_hw_devices $hw_fpga_name] 0]]    
        set_property PROGRAM.FILE $pdi [lindex [get_hw_devices $hw_fpga_name] 0]
        set is_versal true
      } else {
        #unkown
      ...
      if {$is_versal} {
        #program  bitstream first, needed for versal
        # program_hw_devices [lindex [get_hw_devices $hw_fpga_name] 0]; refresh_hw_device [lindex [get_hw_devices $hw_fpga_name] 0];
        TE::VLAB::hw_program_fpga_device false "NA"
      }
